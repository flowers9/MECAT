...
void PackedDB::add_one_seq(const Sequence& seq) {
	...
	const idx_t size((si.size + 3) / 4);
	const idx_t needed_size(db_size + size);
	...
	for (idx_t i(0); i < si.size; ++i, ++db_size) {
		...
		set_char(pac, db_size, c < 4 ? c : ++rand_char & 3);
	}
}
